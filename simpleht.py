...
import sys, SimpleXMLRPCServer, getopt, pickle, time, threading, xmlrpclib, unittest, fcntl, os
...
class SimpleHT:
  ...
  def acquire(self, key):
    if key.data == '/':
      ...
      name = key.data
    path = '/tmp' + name
    self.fd = os.open(path, os.O_CREAT)
...
def serve(port):
  ...
  file_server.register_function(sht.acquire)
  file_server.register_function(sht.release)
